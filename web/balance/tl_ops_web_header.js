...
const tl_ops_web_header_delete = function () {
    ...
    let new_list = res_data.tl_ops_balance_header_list[rule].filter(item=>{
        return !idList.includes(item.id);
    })

    res_data.tl_ops_balance_header_list[rule] = new_list;
...
const tl_ops_web_header_change_rule = function () {
    ...
    res_data.tl_ops_balance_header_rule = rule;
...
const tl_ops_header_data_add_filter = function( data ) {
    ...
    for(let i = 0; i < res_data.tl_ops_balance_header_list[rule].length; i++){
        let obj = res_data.tl_ops_balance_header_list[rule][i];
    ...
    res_data.tl_ops_balance_header_list[rule].push(data.field)

    res_data.tl_ops_balance_header_list[rule].forEach(item=>{
...
const tl_ops_header_data_edit_filter = function( data ) {
    ...
    res_data.tl_ops_balance_header_list[rule] = cur_list;
