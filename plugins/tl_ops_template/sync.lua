-- tl_ops_template_sync
-- en : sync template config
-- zn : 同步、预热插件模板相关数据
-- @corsor iamtsm
...
local sync_fields_export = function ()
    local cache_key = constant.export.cache_key.template
    local constant_data = constant.export.template
    ...
    if not data_str then
        ...
        if not res then
            tlog:err("template sync_fields_export new store err, cache_key=",cache_key,",res=",res)
            return tl_ops_rt.error
        end

        data_str, _ = cache:get(cache_key);

        tlog:dbg("template sync_fields_export new store,  cache_key=",cache_key,",res=",res)
    end

    local data = cjson.decode(data_str);
    if not data and type(data) ~= 'table' then
        tlog:err("template sync_fields_export err,  cache_key=",cache_key,",old=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("template sync_fields_export start,  cache_key=",cache_key,",old=",data)
    ...
    if not res then
        tlog:err("template sync_fields_export err,  cache_key=",cache_key,",res=",res,",new=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("template sync_fields_export done,  cache_key=",cache_key,",new=",data,",add_keys=",add_keys)
