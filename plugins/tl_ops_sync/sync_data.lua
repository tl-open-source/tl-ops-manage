...
local sync_data_balance_api = function () 
    ...
    if not data_str then
        local res, _ = cache_balance_api:set(cache_key_list, cjson.encode(constant_data))
        if not res then
...
local sync_data_balance_body = function () 
    ...
    if not data_str then
        local res, _ = cache_balance_body:set(cache_key_list, cjson.encode(constant_data))
        if not res then
...
local sync_data_balance_cookie = function ()
    ...
    if not data_str then
        local res, _ = cache_balance_cookie:set(cache_key_list, cjson.encode(constant_data))
        if not res then
...
local sync_data_balance_header = function ()
    ...
    if not data_str then
        local res, _ = cache_balance_header:set(cache_key_list, cjson.encode(constant_data))
        if not res then
...
local sync_data_balance_param = function ()
    ...
    if not data_str then
        local res, _ = cache_balance_param:set(cache_key_list, cjson.encode(constant_data))
        if not res then
...
local sync_data_waf_ip = function ()
    ...
    if not data_str then
        local res, _ = cache_waf_ip:set(cache_key_list, cjson.encode(constant_data))
        if not res then
...
local sync_data_waf_api = function ()
    ...
    if not data_str then
        local res, _ = cache_waf_api:set(cache_key_list, cjson.encode(constant_data))
        if not res then
...
local sync_data_waf_cookie = function ()
    ...
    if not data_str then
        local res, _ = cache_waf_cookie:set(cache_key_list, cjson.encode(constant_data))
        if not res then
...
local sync_data_waf_header = function ()
    ...
    if not data_str then
        local res, _ = cache_waf_header:set(cache_key_list, cjson.encode(constant_data))
        if not res then
...
local sync_data_waf_param = function ()
    ...
    if not data_str then
        local res, _ = cache_waf_param:set(cache_key_list, cjson.encode(constant_data))
        if not res then
...
local sync_data_waf_cc = function ()
    ...
    if not data_str then
        local res, _ = cache_waf_cc:set(cache_key_list, cjson.encode(constant_data))
        if not res then
...
-- 插件管理列表
local sync_data_plugins_manage = function ()
    local cache_key_list = constant_plugins_manage.cache_key.list

    -- 静态配置
    local constant_data = constant_plugins_manage.list

    local data_str, _ = cache_plugins_manage:get(cache_key_list);
    if not data_str then
        local res, _ = cache_plugins_manage:set(cache_key_list, cjson.encode(constant_data))
        if not res then
            tlog:err("sync_data_plugins_manage new store data err, res=",res)
            return tl_ops_rt.error
        end

        tlog:dbg("sync_data_plugins_manage new store data, res=",res)
        return tl_ops_rt.ok
    end

    local data = cjson.decode(data_str);
    if not data and type(data) ~= 'table' then
        tlog:err("sync_data_plugins_manage err, old=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("sync_data_plugins_manage start, old=",data)
    ...
    if not res then
        tlog:err("sync_data_plugins_manage err, res=",res,",new=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("sync_data_plugins_manage done, new=",data)
...
function _M:sync_data_module( module )
    ...
    elseif module == 'plugins_manage' then
        return sync_data_plugins_manage()
    else 
        -- plugin
