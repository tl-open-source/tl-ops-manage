...
local sync_fields_export = function ()
    local cache_key = constant.export.cache_key.tracing
    local constant_data = constant.export.tracing
    ...
    if not data_str then
        ...
        if not res then
            tlog:err("tracing sync_fields_export new store err, cache_key=",cache_key,",res=",res)
            return tl_ops_rt.error
        end

        data_str, _ = cache:get(cache_key);

        tlog:dbg("tracing sync_fields_export new store,  cache_key=",cache_key,",res=",res)
    end

    local data = cjson.decode(data_str);
    if not data and type(data) ~= 'table' then
        tlog:err("tracing sync_fields_export err,  cache_key=",cache_key,",old=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tracing sync_fields_export start,  cache_key=",cache_key,",old=",data)
    ...
    if not res then
        tlog:err("tracing sync_fields_export err,  cache_key=",cache_key,",res=",res,",new=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tracing sync_fields_export done,  cache_key=",cache_key,",new=",data,",add_keys=",add_keys)
