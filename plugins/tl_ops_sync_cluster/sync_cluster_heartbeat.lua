...
local sync_cluster_heartbeat_receive = function( ctx )
    ...
    tlog:dbg("sync_cluster_heartbeat_receive heartbeat start, uri=",ctx.uri,",method=",ctx.method,",body=",ngx.req.get_body_data())

    tlog:dbg("sync_cluster_heartbeat_receive heartbeat done")
...
local sync_cluster_heartbeat_send = function(options)
    ...
    for i = 1, #other do
        ...
        local req_data = sync_cluster_data:get_sync_cluster_data_module(modules)
        ...
        local body = "GET " .. path .." HTTP/1.0 " .. req_data .. "\r\n\r\n\r\n"
