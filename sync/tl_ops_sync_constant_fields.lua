...
local tlog = require("utils.tl_ops_utils_log"):new("tl_ops_sync_constant_fields");
...
-- 服务节点数据同步
local tl_ops_sync_constant_fields_service = function ()
    ...
    if not data_str then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_service new store list err, res=",res)
            return tl_ops_rt.error
        end

        data_str, _ = cache_service:get(cache_key);

        tlog:dbg("tl_ops_sync_constant_fields_service new store data, res=",res)
    end
    ...
    if not data_rule_str then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_service new store rule err, res=",res)
            return tl_ops_rt.error
        end

        tlog:dbg("tl_ops_sync_constant_fields_service new store rule, res=",res)
    end

    local data = cjson.decode(data_str);
    if not data and type(data) ~= 'table' then
        tlog:err("tl_ops_sync_constant_fields_service err, old=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_service start, old=",data)
    ...
    if not res then
        tlog:err("tl_ops_sync_constant_fields_service err, res=",res,",new=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_service done, new=",data,",add_keys=",add_keys)
...
-- 健康检查数据同步
local tl_ops_sync_constant_fields_health = function ()
    ...
    if not data_str then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_health new store err, res=",res)
            return tl_ops_rt.error
        end

        data_str, _ = cache_health:get(cache_key);

        tlog:dbg("tl_ops_sync_constant_fields_health new store, res=",res)
    end

    local data = cjson.decode(data_str);
    if not data and type(data) ~= 'table' then
        tlog:err("tl_ops_sync_constant_fields_health err, old=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_health start, old=",data)
    ...
    if not res then
        tlog:err("tl_ops_sync_constant_fields_health err, res=",res,",new=",data)
        return tl_ops_rt.error
    end
    ...
    tlog:dbg("tl_ops_sync_constant_fields_health done, new=",data,",add_keys=",add_keys)
...
-- 熔断数据同步
local tl_ops_sync_constant_fields_limit = function ()
    ...
    if not data_str then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_limit new store err, res=",res)
            return tl_ops_rt.error
        end

        data_str, _ = cache_limit:get(cache_key);

        tlog:dbg("tl_ops_sync_constant_fields_limit new store, res=",res)
    end

    local data = cjson.decode(data_str);
    if not data and type(data) ~= 'table' then
        tlog:err("tl_ops_sync_constant_fields_limit err, old=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_limit start, old=",data)
    ...
    if not res then
        tlog:err("tl_ops_sync_constant_fields_limit err, res=",res,",new=",data)
        return tl_ops_rt.error
    end
    ...
    tlog:dbg("tl_ops_sync_constant_fields_limit done, new=",data,",add_keys=",add_keys)

    return tl_ops_rt.ok
end

-- 限流数据同步 token
local tl_ops_sync_constant_fields_limit_token = function ()
    ...
    if not data_str then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_limit_token new store err, res=",res)
            return tl_ops_rt.error
        end

        data_str, _ = cache_limit:get(cache_key);

        tlog:dbg("tl_ops_sync_constant_fields_limit_token new store, res=",res)
    end

    local data = cjson.decode(data_str);
    if not data and type(data) ~= 'table' then
        tlog:err("tl_ops_sync_constant_fields_limit_token err, old=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_limit_token start, old=",data)
    ...
    local res = cache_limit:set(cache_key, cjson.encode(data))
    if not res then
        tlog:err("tl_ops_sync_constant_fields_limit_token err, res=",res,",new=",data)
        return tl_ops_rt.error
    end
    ...
    tlog:dbg("tl_ops_sync_constant_fields_limit_token done, new=",data,",add_keys=",add_keys)

    return tl_ops_rt.ok
end

-- 限流数据同步 leak
local tl_ops_sync_constant_fields_limit_leak = function ()
    ...
    if not data_str then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_limit_leak new store err, res=",res)
            return tl_ops_rt.error
        end

        data_str, _ = cache_limit:get(cache_key);

        tlog:dbg("tl_ops_sync_constant_fields_limit_leak new store, res=",res)
    end

    local data = cjson.decode(data_str);
    if not data and type(data) ~= 'table' then
        tlog:err("tl_ops_sync_constant_fields_limit_leak err, old=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_limit_leak start, old=",data)
    ...
    local res = cache_limit:set(cache_key, cjson.encode(data))
    if not res then
        tlog:err("tl_ops_sync_constant_fields_limit_leak err, res=",res,",new=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_limit_leak done, new=",data,",add_keys=",add_keys)
...
-- 路由配置数据同步
local tl_ops_sync_constant_fields_balance = function ()
    ...
    if not data_str then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_balance new store err, res=",res)
            return tl_ops_rt.error
        end

        data_str, _ = cache_balance:get(cache_key);

        tlog:dbg("tl_ops_sync_constant_fields_balance new store, res=",res)
    end

    local data = cjson.decode(data_str);
    if not data and type(data) ~= 'table' then
        tlog:err("tl_ops_sync_constant_fields_balance err, old=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_balance start, old=",data)
    ...
    if not res then
        tlog:err("tl_ops_sync_constant_fields_balance err, res=",res,",new=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_balance done, new=",data,",add_keys=",add_keys)

    return tl_ops_rt.ok
end

-- api策略数据同步
local tl_ops_sync_constant_fields_balance_api = function ()
    ...
    if not data_str then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_balance_api new store data err, res=",res)
            return tl_ops_rt.error
        end

        data_str, _ = cache_balance_api:get(cache_key_list)

        tlog:dbg("tl_ops_sync_constant_fields_balance_api new store data, res=",res)
    end
    ...
    if not data_rule_str then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_balance_api new store rule err, res=",res)
            return tl_ops_rt.error
        end

        tlog:dbg("tl_ops_sync_constant_fields_balance_api new store rule, res=",res)
    end

    local data = cjson.decode(data_str);
    if not data and type(data) ~= 'table' then
        tlog:err("tl_ops_sync_constant_fields_balance_api err, old=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_balance_api start, old=",data)
    ...
    if not res then
        tlog:err("tl_ops_sync_constant_fields_balance_api err, res=",res,",new=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_balance_api done, new=",data,",add_keys=",add_keys)

    return tl_ops_rt.ok
end

-- cookie策略数据同步
local tl_ops_sync_constant_fields_balance_cookie = function ()
    ...
    if not data_str then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_balance_cookie new store data err, res=",res)
            return tl_ops_rt.error
        end

        data_str, _ = cache_balance_cookie:get(cache_key_list);

        tlog:dbg("tl_ops_sync_constant_fields_balance_cookie new store data, res=",res)
    end
    ...
    if not data_rule_str then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_balance_cookie new store rule err, res=",res)
            return tl_ops_rt.error
        end
    ...
    local data = cjson.decode(data_str);
    if not data and type(data) ~= 'table' then
        tlog:err("tl_ops_sync_constant_fields_balance_cookie err, old=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_balance_cookie start, old=",data)
    ...
    if not res then
        tlog:err("tl_ops_sync_constant_fields_balance_cookie err, res=",res,",new=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_balance_cookie done, new=",data,",add_keys=",add_keys)

    return tl_ops_rt.ok
end

-- header策略数据同步
local tl_ops_sync_constant_fields_balance_header = function ()
    ...
    if not data_str then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_balance_header new store data err, res=",res)
            return tl_ops_rt.error
        end

        data_str, _ = cache_balance_header:get(cache_key_list);

        tlog:dbg("tl_ops_sync_constant_fields_balance_header new store data, res=",res)
    end
    ...
    if not data_rule_str then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_balance_header new store rule err, res=",res)
            return tl_ops_rt.error
        end

        tlog:dbg("tl_ops_sync_constant_fields_balance_header new store rule, res=",res)
    end


    local data = cjson.decode(data_str);
    if not data and type(data) ~= 'table' then
        tlog:err("tl_ops_sync_constant_fields_balance_header err, old=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_balance_header start, old=",data)
    ...
    if not res then
        tlog:err("tl_ops_sync_constant_fields_balance_header err, res=",res,",new=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_balance_header done, new=",data,",add_keys=",add_keys)

    return tl_ops_rt.ok
end

-- param策略数据同步
local tl_ops_sync_constant_fields_balance_param = function ()
    ...
    if not data_str then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_balance_param new store data err, res=",res)
            return tl_ops_rt.error
        end

        data_str, _ = cache_balance_param:get(cache_key_list);

        tlog:dbg("tl_ops_sync_constant_fields_balance_param new store data, res=",res)
    end
    ...
    if not data_rule_str then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_balance_param new store rule err, res=",res)
            return tl_ops_rt.error
        end

        tlog:dbg("tl_ops_sync_constant_fields_balance_param new store rule, res=",res)
    end

    local data = cjson.decode(data_str);
    if not data and type(data) ~= 'table' then
        tlog:err("tl_ops_sync_constant_fields_balance_param err, old=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_balance_param start, old=",data)
    ...
    if not res then
        tlog:err("tl_ops_sync_constant_fields_balance_param err, res=",res,",new=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_balance_param done, new=",data,",add_keys=",add_keys)
...
-- waf配置数据同步
local tl_ops_sync_constant_fields_waf = function ()
    ...
    if not data_str then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_waf new store err, res=",res)
            return tl_ops_rt.error
        end

        data_str, _ = cache_waf:get(cache_key);

        tlog:dbg("tl_ops_sync_constant_fields_waf new store, res=",res)
    end

    local data = cjson.decode(data_str);
    if not data and type(data) ~= 'table' then
        tlog:err("tl_ops_sync_constant_fields_waf err, old=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_waf start, old=",data)
    ...
    if not res then
        tlog:err("tl_ops_sync_constant_fields_waf err, res=",res,",new=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_waf done, new=",data,",add_keys=",add_keys)

    return tl_ops_rt.ok
end

-- api waf规则数据同步
local tl_ops_sync_constant_fields_waf_api = function ()
    ...
    if not data_str then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_waf_api new store data err, res=",res)
            return tl_ops_rt.error
        end

        data_str, _ = cache_waf_api:get(cache_key_list)

        tlog:dbg("tl_ops_sync_constant_fields_waf_api new store data, res=",res)
    end
    ...
    if not data_scope then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_waf_api new store scope err, res=",res)
            return tl_ops_rt.error
        end

        tlog:dbg("tl_ops_sync_constant_fields_waf_api new store scope, res=",res)
    end
    ...
    if not data_open then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_waf_api new store open err, res=",res)
            return tl_ops_rt.error
        end

        tlog:dbg("tl_ops_sync_constant_fields_waf_api new store open, res=",res)
    end

    local data = cjson.decode(data_str);
    if not data and type(data) ~= 'table' then
        tlog:err("tl_ops_sync_constant_fields_waf_api err, old=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_waf_api start, old=",data)
    ...
    if not res then
        tlog:err("tl_ops_sync_constant_fields_waf_api err, res=",res,",new=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_waf_api done, new=",data,",add_keys=",add_keys)

    return tl_ops_rt.ok
end

-- ip waf规则数据同步
local tl_ops_sync_constant_fields_waf_ip = function ()
    ...
    if not data_str then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_waf_ip new store data err, res=",res)
            return tl_ops_rt.error
        end

        data_str, _ = cache_waf_ip:get(cache_key_list)

        tlog:dbg("tl_ops_sync_constant_fields_waf_ip new store data, res=",res)
    end
    ...
    if not data_scope then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_waf_ip new store scope err, res=",res)
            return tl_ops_rt.error
        end

        tlog:dbg("tl_ops_sync_constant_fields_waf_ip new store scope, res=",res)
    end
    ...
    if not data_open then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_waf_ip new store open err, res=",res)
            return tl_ops_rt.error
        end

        tlog:dbg("tl_ops_sync_constant_fields_waf_ip new store open, res=",res)
    end

    local data = cjson.decode(data_str);
    if not data and type(data) ~= 'table' then
        tlog:err("tl_ops_sync_constant_fields_waf_ip err, old=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_waf_ip start, old=",data)
    ...
    if not res then
        tlog:err("tl_ops_sync_constant_fields_waf_ip err, res=",res,",new=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_waf_ip done, new=",data,",add_keys=",add_keys)

    return tl_ops_rt.ok
end

-- cookie waf规则数据同步
local tl_ops_sync_constant_fields_waf_cookie = function ()
    ...
    if not data_str then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_waf_cookie new store data err, res=",res)
            return tl_ops_rt.error
        end

        data_str, _ = cache_waf_cookie:get(cache_key_list)

        tlog:dbg("tl_ops_sync_constant_fields_waf_cookie new store data, res=",res)
    end
    ...
    if not data_scope then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_waf_cookie new store scope err, res=",res)
            return tl_ops_rt.error
        end

        tlog:dbg("tl_ops_sync_constant_fields_waf_cookie new store scope, res=",res)
    end
    ...
    if not data_open then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_waf_cookie new store open err, res=",res)
            return tl_ops_rt.error
        end

        tlog:dbg("tl_ops_sync_constant_fields_waf_cookie new store open, res=",res)
    end

    local data = cjson.decode(data_str);
    if not data and type(data) ~= 'table' then
        tlog:err("tl_ops_sync_constant_fields_waf_cookie err, old=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_waf_cookie start, old=",data)
    ...
    if not res then
        tlog:err("tl_ops_sync_constant_fields_waf_cookie err, res=",res,",new=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_waf_cookie done, new=",data,",add_keys=",add_keys)

    return tl_ops_rt.ok
end

-- header waf规则数据同步
local tl_ops_sync_constant_fields_waf_header = function ()
    ...
    if not data_str then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_waf_header new store data err, res=",res)
            return tl_ops_rt.error
        end

        data_str, _ = cache_waf_header:get(cache_key_list)

        tlog:dbg("tl_ops_sync_constant_fields_waf_header new store data, res=",res)
    end
    ...
    if not data_scope then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_waf_header new store scope err, res=",res)
            return tl_ops_rt.error
        end

        tlog:dbg("tl_ops_sync_constant_fields_waf_header new store scope, res=",res)
    end
    ...
    if not data_open then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_waf_header new store open err, res=",res)
            return tl_ops_rt.error
        end

        tlog:dbg("tl_ops_sync_constant_fields_waf_header new store open, res=",res)
    end

    local data = cjson.decode(data_str);
    if not data and type(data) ~= 'table' then
        tlog:err("tl_ops_sync_constant_fields_waf_header err, old=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_waf_header start, old=",data)
    ...
    if not res then
        tlog:err("tl_ops_sync_constant_fields_waf_header err, res=",res,",new=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_waf_header done, new=",data,",add_keys=",add_keys)

    return tl_ops_rt.ok
end

-- param waf规则数据同步
local tl_ops_sync_constant_fields_waf_param = function ()
    ...
    if not data_str then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_waf_param new store data err, res=",res)
            return tl_ops_rt.error
        end

        data_str, _ = cache_waf_param:get(cache_key_list)

        tlog:dbg("tl_ops_sync_constant_fields_waf_param new store data, res=",res)
    end
    ...
    if not data_scope then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_waf_param new store scope err, res=",res)
            return tl_ops_rt.error
        end

        tlog:dbg("tl_ops_sync_constant_fields_waf_param new store scope, res=",res)
    end
    ...
    if not data_open then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_waf_param new store open err, res=",res)
            return tl_ops_rt.error
        end

        tlog:dbg("tl_ops_sync_constant_fields_waf_param new store open, res=",res)
    end

    local data = cjson.decode(data_str);
    if not data and type(data) ~= 'table' then
        tlog:err("tl_ops_sync_constant_fields_waf_param err, old=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_waf_param start, old=",data)
    ...
    if not res then
        tlog:err("tl_ops_sync_constant_fields_waf_param err, res=",res,",new=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_waf_param done, new=",data,",add_keys=",add_keys)

    return tl_ops_rt.ok
end

-- cc waf规则数据同步
local tl_ops_sync_constant_fields_waf_cc = function ()
    ...
    if not data_str then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_waf_cc new store data err, res=",res)
            return tl_ops_rt.error
        end

        data_str, _ = cache_waf_cc:get(cache_key_list)

        tlog:dbg("tl_ops_sync_constant_fields_waf_cc new store data, res=",res)
    end
    ...
    if not data_scope then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_waf_cc new store scope err, res=",res)
            return tl_ops_rt.error
        end

        tlog:dbg("tl_ops_sync_constant_fields_waf_cc new store scope, res=",res)
    end
    ...
    if not data_open then
        ...
        if not res then
            tlog:err("tl_ops_sync_constant_fields_waf_cc new store open err, res=",res)
            return tl_ops_rt.error
        end

        tlog:dbg("tl_ops_sync_constant_fields_waf_cc new store open, res=",res)
    end

    local data = cjson.decode(data_str);
    if not data and type(data) ~= 'table' then
        tlog:err("tl_ops_sync_constant_fields_waf_cc err, old=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_waf_cc start, old=",data)
    ...
    if not res then
        tlog:err("tl_ops_sync_constant_fields_waf_cc err, res=",res,",new=",data)
        return tl_ops_rt.error
    end

    tlog:dbg("tl_ops_sync_constant_fields_waf_cc done, new=",data,",add_keys=",add_keys)
...
function _M:tl_ops_sync_constant_fields_module( module )

    if module == 'service' then
        return tl_ops_sync_constant_fields_service()
    end

    if module == 'health' then
        return tl_ops_sync_constant_fields_health()
    end

    if module == 'limit-fuse' then
        return tl_ops_sync_constant_fields_limit()
    end

    if module == 'limit-token' then
        return tl_ops_sync_constant_fields_limit_token()
    end

    if module == 'limit-leak' then
        return tl_ops_sync_constant_fields_limit_leak()
    end

    if module == 'balance' then
        return tl_ops_sync_constant_fields_balance()
    end

    if module == 'balance-api' then
        return tl_ops_sync_constant_fields_balance_api()
    end

    if module == 'balance-cookie' then
        return tl_ops_sync_constant_fields_balance_cookie()
    end

    if module == 'balance-header' then
        return tl_ops_sync_constant_fields_balance_header()
    end

    if module == 'balance-param' then
        return tl_ops_sync_constant_fields_balance_param()
    end

    if module == 'waf' then
        return tl_ops_sync_constant_fields_waf()
    end

    if module == 'waf-api' then
        return tl_ops_sync_constant_fields_waf_api()
    end

    if module == 'waf-ip' then
        return tl_ops_sync_constant_fields_waf_ip()
    end
    
    if module == 'waf-header' then
        return tl_ops_sync_constant_fields_waf_header()
    end

    if module == 'waf-cookie' then
        return tl_ops_sync_constant_fields_waf_cookie()
    end

    if module == 'waf-param' then
        return tl_ops_sync_constant_fields_waf_param()
    end

    if module == 'waf-cc' then
        return tl_ops_sync_constant_fields_waf_cc()
    end
end
