...
local rule, _ = cache:get(tl_ops_constant_balance.cache_key.service_rule);
if not rule or rule == nil then
    tl_ops_utils_func:set_ngx_req_return_ok(tl_ops_rt.not_found ,"not found rule", _);
    return;
end

local list, _ = cache:get(tl_ops_constant_balance.cache_key.service_list);
...
local res_data = {}
res_data[tl_ops_constant_balance.cache_key.service_rule] = rule
res_data[tl_ops_constant_balance.cache_key.service_list] = cjson.decode(list)
