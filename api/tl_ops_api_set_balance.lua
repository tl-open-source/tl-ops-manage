...
if not leak_limit or leak_limit == nil then
    tl_ops_utils_func:set_ngx_req_return_ok(tl_ops_rt.args_error ,"args err6", _);
    return;
end
...
if not offline or offline == nil then
    tl_ops_utils_func:set_ngx_req_return_ok(tl_ops_rt.args_error ,"args err7", _);
    return;
end
...
if service_empty < 200 or service_empty >= 600 then
    tl_ops_utils_func:set_ngx_req_return_ok(tl_ops_rt.args_error ,"args err8", _);
    return;
end

mode_empty = tonumber(mode_empty)
if service_empty < 200 or service_empty >= 600 then
    tl_ops_utils_func:set_ngx_req_return_ok(tl_ops_rt.args_error ,"args err9", _);
    return;
end

host_empty = tonumber(host_empty)
if service_empty < 200 or service_empty >= 600 then
    tl_ops_utils_func:set_ngx_req_return_ok(tl_ops_rt.args_error ,"args err10", _);
    return;
end

host_pass = tonumber(host_pass)
if service_empty < 200 or service_empty >= 600 then
    tl_ops_utils_func:set_ngx_req_return_ok(tl_ops_rt.args_error ,"args err11", _);
    return;
end

token_limit = tonumber(token_limit)
if service_empty < 200 or service_empty >= 600 then
    tl_ops_utils_func:set_ngx_req_return_ok(tl_ops_rt.args_error ,"args err12", _);
    return;
end

leak_limit = tonumber(leak_limit)
if service_empty < 200 or service_empty >= 600 then
    tl_ops_utils_func:set_ngx_req_return_ok(tl_ops_rt.args_error ,"args err13", _);
    return;
end

offline = tonumber(offline)
if service_empty < 200 or service_empty >= 600 then
    tl_ops_utils_func:set_ngx_req_return_ok(tl_ops_rt.args_error ,"args err14", _);
    return;
end
